...
\subsection{Introduction to Arduino hardware}
...
\clearpage
\subsection{Using the Arduino IDE}

The Arduino IDE is shown in Fig~\ref{fig:arduinoide}. The programs in Arduino are sometimes called "sketches" and are shown in the middle region with the white background. The black background region at the bottom is for compiler messages. 

\begin{figure}[h!]
	\begin{center}
		\includegraphics[width=4in]{./ide.pdf}
		\caption{Arduino IDE}
...
\subsection{Adding an LED to the Arduino}
...
\begin{figure}[h]
	\begin{center}
		\includegraphics[width=4in]{./led.pdf}
		\caption{Connecting an LED to pin 13}
		\label{fig:led}
	\end{center}
\end{figure}
...
\subsection{Writing to the display}
...
\subsection{Button interactions}
...
\subsection{Serial pass through from IDE to LCD}
...
\subsection{Reading with analogRead()}
...
\subsection{Reading with digitalRead()}


Alternatively, you can read the sensor as a logic level input.  For many applications, this will suffice - we only need to know if something is either blocking, or not blocking sensor.  Try using the {\em digitalRead()} command with the optical sensor - refer to the code in Listing ~\ref{opticalDigitalRead} for an example of setting this up!


\begin{lstlisting}[language={C},caption={Reading an Optical Sensor with digitalRead},label={opticalDigitalRead}]
...
The difference between an {\em analogRead()} and {\em digitalRead()} may seem trivial, but to illustrate the difference in hardware requirements, try playing with the code in ~\ref{timingAnalysis}

\begin{lstlisting}[language={C},caption={Timing analysis base code},label={timingAnalysis}]
...
void loop()
{
  ...
  t2 = millis();
  tdelta = t2 - t1;
  Serial.print("Time for 1000 analogRead's is approximately: ");
  Serial.print(tdelta);
  Serial.println(" mS!");
...
\subsection{Description of Servo Motors}
...
\subsubsection{Connecting to a Servo Motor}
...
\subsubsection{Sending commands to the Servo}
...
%\section{Exercises}
\subsection{Control position with left/right buttons}
...
\subsection{Control step size with up/down buttons}
...
\subsection{Control via Serial Port}
...
\subsection{Control via LCD interface}
...
\subsection{Control via optical sensor}
